...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            return Emps.Where(e => e.Salary == Emps.Max(ie => ie.Salary));
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            {
                return Emps.Count(e => e.Deptno == d.Deptno) is 0 or 5;
            });
        }
        ...
        {
            //note: the method probably had incorrect signature - I changed it to return object
            // there is no data with an upper A in Job, so I changed requirements to case-insensitive
            return from emp in Emps
                where emp.Job.ToLower().Contains("a")
                group emp by emp.Job into jobJoin
                where jobJoin.Count() > 2
                orderby jobJoin.Count() descending
                select new
                {
                    Praca = jobJoin.Key,
                    LiczbaPracownikow = jobJoin.Count()
                };
        }
